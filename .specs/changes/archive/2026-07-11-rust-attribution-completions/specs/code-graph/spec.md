...
> Sequencing: this delta modifies the Guarded positional join as amended by the `python-attribution-completions` change; that change syncs first.

## MODIFIED Requirements

### Requirement: Guarded positional join

> Previously: post python-attribution-completions, Rust had four kind-scoped rules — crate-root, operator-desugar, module-span, self-keyword; the self-keyword rule compared only against the implementation's self type; no range, tuple-index, or path-keyword evidence.
...
Serves: operator-aware-references, idiomatic-rust-evidence
