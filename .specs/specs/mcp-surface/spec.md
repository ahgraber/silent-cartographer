...
Defines the MCP server that exposes the CLI as named tools over stdio.
The tools mirror the commands one to one and return each command's structured answer unmodified, so provenance, freshness, typed absence, and heuristic-grade labels reach the caller intact.
The contract covers what the surface advertises, how every invocation is scoped to one workspace, how a command's exit taxonomy reaches the caller as typed tool errors, the startup gate holding the tools to the installed command surface, the consent a state-changing operation requires, and service to clients on either side of the `2026-07-28` protocol revision.
